...
- The namespace is `(effective originator, idempotencyKey)` across both active and archived rollouts. The first creation writes and flushes that binding before its JSON-RPC response is sent. Concurrent calls and retries after an app-server restart therefore return/resume the same thread id.
