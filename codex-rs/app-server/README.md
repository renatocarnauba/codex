...
`clientInfo` is scoped to the transport connection. When several clients share one WebSocket app-server,
threads created by each connection retain that connection's `clientInfo.name` as their originator (unless a
recognized service name, persisted or inherited originator, or explicit originator override takes precedence).
The server does not reuse the first initialized client's identity for later connections.
...
- `turn/steer` — add user input to an already in-flight regular turn without starting a new turn; returns the active `turnId` that accepted the input. Experimental `idempotencyKey` durably journals acceptance so concurrent, response-lost, and post-restart retries do not enqueue the same steering input again. `clientUserMessageId` is optional; when supplied, the corresponding `userMessage` item echoes it as `clientId`. Review and manual compaction turns reject `turn/steer`.
...
- `server/connection/list` — experimental, read-only diagnostic inventory of initialized live app-server connections. Each row exposes only a process-local opaque id, the exact initialized `clientInfo.name`, request-attestation capability, and whether the row is the caller's connection. It never exposes authorization material or request payloads and is intended to verify shared-daemon client identity (for example, that `Codex Desktop` is connected alongside an integration).
...
Experimental durable creation keys have these exact semantics:

- `idempotencyKey` is a non-blank string of at most 256 characters and cannot be used with an ephemeral thread or fork.
- The namespace is `(effective originator, idempotencyKey)` across both `thread/start` and `thread/fork`, and across active and archived threads. Operation kind is validated payload, not a second namespace. The first creation reserves the key in SQLite, writes and flushes canonical `SessionMeta`, then commits the reservation before its JSON-RPC response is sent. Concurrent calls and retries after an app-server restart therefore return/resume the same thread id.
- If reconciliation archived the created thread before the retry, app-server unarchives and resumes that same thread. Multiple durable matches fail closed instead of selecting one.
- A `thread/start` binding also fixes the exact requested `cwd`, `threadSource`, and `serviceName`. A `thread/fork` binding additionally fixes the resolved source thread id plus `lastTurnId`/`beforeTurnId`. Reusing a key with different immutable identity is rejected and never creates a replacement.
- Originator namespaces are isolated. A connection cannot claim another originator's creation by repeating its key. Idempotent fork is allowed only when the calling connection's effective originator owns the source thread.
- The normal lookup path is the SQLite primary key and resolves the current rollout path by joining `threads`; it must never scan rollout files or cache a path that archive/unarchive can move. Migration projects legacy `SessionMeta` through the existing leased startup backfill before RPC is accepted. Pending reservations are owner-cleaned on flush failure or lease-reconciled at startup, and bindings are removed only by definitive thread deletion. Future implementations must not turn a missing path into permission to delete/rebind a key, add an O(N) fallback, or emit `thread/started` before reserve → canonical flush → commit completes.

Experimental `turn/start.idempotencyKey` applies the same validation and originator namespace to a turn acceptance. Its binding also fixes the target thread and a canonical fingerprint of the complete request. App-server flushes a reserved turn id to the thread rollout before submitting input to core. Concurrent retries return that id without a second submission; after restart, a reserved action is safely submitted with the same id, while an in-progress or terminal action is returned as-is. Reusing the key with any changed field is rejected. Archived threads retain their completed turn binding, but retrying it does not unarchive or re-execute the thread. Ephemeral threads and threads owned by another originator reject durable turn keys. Multiple durable matches fail closed.

Experimental `turn/steer.idempotencyKey` uses a separate action namespace with the same key validation, owner checks, and canonical full-request fingerprint. The reservation is flushed before the steering input is enqueued; concurrent calls and retries after a response loss or daemon restart return the originally accepted `turnId` without enqueueing again. A normal core rejection writes and flushes a cancellation record so the key is not poisoned and can be retried. Changed requests are rejected, and ephemeral or foreign-originator threads cannot use durable steering keys.

`turn/started.clientName` is bound to the accepted request's `turnId`. Non-input `item/started.clientName` keeps that turn owner, while each canonical `userMessage` item persists the authenticated `turn/start` or `turn/steer` connection that submitted that exact input. The durable `InputAttribution` reservation is crash-audit/idempotency state only; it must never be used to authenticate or backfill a user item. A canonical user item without `clientName` stays unattributed (`null`/system), even when an orphaned reservation or turn owner exists. These values must not be read from the thread's mutable config snapshot: a losing or steering request on another connection can update connection-scoped settings while the winning turn is still emitting events.

Recursive `thread/archive` is a lifecycle operation. It shares one permit with every app-server transition from idle/not-loaded to active (`turn/start` and inline/detached `review/start`), validates the complete root/descendant tree, and rejects an active or unreadable tree before moving any rollout. New turn-producing RPCs must acquire the same permit and publish their active marker before releasing it; checking `ThreadStatus` without serializing the transition reintroduces the start/archive TOCTOU race.

Thread display metadata is deliberately split rather than represented by a `description` field. `thread.name` is the persisted human title and is updated with `thread/setName`; `thread.preview` is the durable first-user-message preview. Current Codex Desktop task discovery chooses `name`, then `preview`, then `cwd`, then `id`. It groups project-backed tasks from the thread `cwd`. Integrations should therefore use the original Desktop project cwd, call `thread/setName` with a human title after an idempotent start/fork response, and make the first user turn a human-readable description. Internal ids, worktree paths, and machine control payloads must not be used as those display values.
...
Use `thread/archive` to move the persisted rollout (stored as a JSONL file on disk) and its readable, quiescent spawned-descendant tree into the archived sessions directory. The request fails before mutation when any candidate has an active turn or when the complete tree cannot be read.
...
- `turn/started` — `{ turn, clientName }` with the turn id, empty `items`, and `status: "inProgress"`.
  `clientName` identifies the app-server connection that submitted `turn/start`.
...
- `item/started` — emits the full `item` and `clientName` when a new unit of work begins so the UI can render it immediately; the `item.id` in this payload matches the `itemId` used by deltas. For user input, `clientName` identifies the connection that submitted `turn/start` or `turn/steer`; it can be `null` for events that do not have a connection-scoped origin.
...
Dynamic tools are owned by the connection that registered them on `thread/start`. In a shared WebSocket
app-server, `item/tool/call` is sent only to that owner, even when another subscribed client starts the turn;
item lifecycle notifications remain visible to thread subscribers. If the owner disconnects, the request is
kept pending instead of being broadcast. A newly initialized connection with the same exact
`clientInfo.name` reclaims the capability when it resumes the thread, and the pending request is replayed to
that connection. A different client such as Codex Desktop never receives the callback. Persisted threads
recover the same owner identity after app-server restart; threads created before ownership metadata existed
retain the legacy all-subscriber fallback for compatibility.
